calc: function(arg, by, room) {
        if (!Bot.hasRank(by, '+%@#&~★')) room = ',' + by;
        if (!arg || !toId(arg)) return false;
        var parts = arg.replace(/, /g, ',').split(',');
        if (parts.length < 2) return Bot.say(by, room, 'Use ' + config.commandcharacter[0] + 'calc [attacker details], [defender details], [extras - weather, etc]. To denote a berry that weakens a SE attack, simply put ``berry`` as one of the defender\'s details. For a gem just put ``gem`` for attacker.');
        ...
        if (!attacker.id || !attacker.move) return Bot.say(by, room, 'You have not included the attacking Pokemon/Move yet!');
        ...
        if (!defender.id) return Bot.say(by, room, 'You have not included the defending Pokemon/Move yet!');
